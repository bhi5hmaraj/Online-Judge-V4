...
class NMNMX {
	...
	static int binomialCoeff[][] = new int[MAX][MAX];
	static final int expMod = (int) (1e9) + 6;
	...
	static {
		...
		for(int i = 2; i < MAX; i++)
		    for(int j = 1; j < MAX; j++)
		        binomialCoeff[i][j] = (binomialCoeff[i - 1][j] + binomialCoeff[i - 1][j - 1]) % expMod;
	
	}
	
	static int expAdd(int a, int b) {
		return (a + b) % expMod;
	}
    
	static int expSub(int a, int b) {
		return (a - b + expMod) % expMod;
	}
    ...
    private static void solve() {
    	...
    	while(T-->0) {
    		...
    		int product = 1;
    		int totalWaysEach = nCr(N - 1, K - 1);
    		
    		for(int i = 0; i < N; i++) {
    			int left = nCr(i, K - 1);
    			int right = nCr(N - i - 1, K - 1);
    			int cntReq = expSub(totalWaysEach, expAdd(left, right));
    			product = MMInt.mul(product, MMInt.modPow(arr[i], cntReq));
    		}

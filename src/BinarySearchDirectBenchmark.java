...
public class BinarySearchDirectBenchmark {
    ...
    static int upperBound(int key) // Number of elements LESS THAN OR EQUAL TO
                                   // key
    {
    ...
    private static void solve1(FastScanner s1, FastWriter out) {
        ...
        Arrays.sort(arr1);
        double copy[] = new double[arr1.length];
        for (int i = 0; i < m; i++)
            copy[i] = (double) arr1[i];
        for (int i = 0; i < n; i++)
            out.print((-(Arrays.binarySearch(copy, (double) arr2[i] + 0.5) + 1)) + " ");

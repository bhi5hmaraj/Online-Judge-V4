import java.util.*;
import java.io.*;
class DIVMAC   // Segtrees are faster than any other method (sparse table) for rmq . They just use 1D array
{
	...
	static class SegmentTreePrime
	{
		int tree[];
		int size;
		boolean lazy[];
		int len;
		SegmentTreePrime(int arr[]) //ar is one based indexing
		{
			len = arr.length - 1;	    
		...
		int query(int node , int L , int R, int nl, int nr)
		{
			...
			int mid = (nl + nr) / 2;
			tree[node] = lazy[node] ? 1 : tree[node];
			// lazy[node] = false;
			if(nl == L && nr == R)
				return tree[node];
			else if(R <= mid)
	...
	static class SegmentTreeRMQ
	{
		...
		int size;
		int len;
		SegmentTreeRMQ(int arr[]) //ar is one based indexing
		{
			len = arr.length - 1;	    
			for(size=1;size<len;size <<= 1)
				;
	...
	static class SegmentTree
	{
		...
		void upd(int node , int nl , int nr){
			if(sync[node]){
				// System.out.println("[" + nl + ", " + nr + "]" + isInternal(node));
		...
		int update(int node,int L, int R,int nl,int nr)
		{
			...
			// System.out.println("tree[2*node] " + tree[2*node] + " tree[(2*node)+1] " + tree[(2*node)+1]);
	...
	private static void solve(FastScanner s1, PrintWriter out){
		...
		while(t-->0)
		{
			...
			if(flag){
				SegmentTreePrime tree = new SegmentTreePrime(arr);
				while(Q-->0){
					int type = s1.nextInt();
					int L = s1.nextInt();
					int R = s1.nextInt();
			...
			//MAX_POW++;
			//System.out.println("max pow " + MAX_POW);
			//sparse = new SparseTable1D[MAX_POW];
			...
			//System.out.print("Time for seg tree rmq table " + ((t2 - t1) / 1e9) + " s ");
			//System.out.println();
			...
			//System.out.println("Time for queries " + ((t3 - t2) / 1e9) + " s");
			//}
